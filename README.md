...
- **`/ark:pr`** — stage, commit, push the current branch, and open a GitHub PR with an auto-generated description that follows the repo's PR template. Sizes the unreviewed diff first and, when it is large or touches a sensitive area (auth, secrets, migrations, infra, concurrency), asks once whether to run `/ark:polish` (full or `--quick`) before pushing; small diffs push straight away. Immediately before the push it runs [`scan-secrets.sh`](plugins/ark/skills/pr/scripts/scan-secrets.sh) over every commit's patch in the branch range and refuses to push on a hit.
...
│           ├── pr/
│           │   ├── SKILL.md
│           │   └── scripts/     # Pre-push secret scan + its test
│           │       ├── scan-secrets.sh
